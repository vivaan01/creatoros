...
import {
  ...
  MessageSquare, FileText, LogOut, Sprout, Menu, X, MoreHorizontal,
} from "lucide-react";
...
// Bottom tab bar — 4 primary + "More" slot
const BOTTOM_TABS = [
      ...
      <div className="md:hidden fixed bottom-0 left-0 right-0 z-40 bg-card/95 backdrop-blur-md border-t border-border/60">
        <div className="flex items-stretch">
          {BOTTOM_TABS.map((tab) => {
            ...
            const colors = NAV_COLORS[tab.href] ?? { text: "text-foreground", glow: "" };
            return (
              <Link
                key={tab.href}
                href={tab.href}
              ...
              >
                <tab.icon className={`h-5 w-5 transition-all ${isActive ? `${colors.text} drop-shadow-[0_0_6px_currentColor]` : ""}`} />
